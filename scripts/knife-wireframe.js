// ==UserScript==
// @name         Knife Wireframe
// @namespace    https://kircraft.io/
// @version      0.2.22
// @description  Display knife model as a wireframe. Use at your own risk.
// @author       pseudoical
...
(async function () {
    ...
    const canvas = await debug(`querySelector("canvas")`, () => {
        return getElement(() => document.querySelector("canvas"));
    });

    const gl = canvas.getContext("webgl2");

    if (gl === null) {
        console.error("gl === null");
        return;
    }

    const gl_drawElements = gl.drawElements;

    gl.drawElements = function (mode, count, type, offset) {
        // Target values identified by Spector.js.
        // See: https://spector.babylonjs.com/
        const isTarget = mode === 4 && count === 3867 && type === 5123 && offset === 0;

        if (isTarget) {
            mode = 1;
        }

        return gl_drawElements.call(this, mode, count, type, offset);
    };
})();
