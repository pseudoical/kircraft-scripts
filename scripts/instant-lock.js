...
// @version      0.2.22
// @description  Automatically confirm lock when locking an item.
// @author       pseudoical
...
(async function () {
     ...
     * @param {Node} target
     */
    async function tryClickLockOk(target) {
    ...
    const observer = new MutationObserver(async (mutations) => {
        for (const mutation of mutations) {
            await tryClickLockOk(mutation.target);
        }
    });

    const lockConfirm = await debug(`querySelector(".ptk-lock-confirm"))`, async () => {
        const element = await getElement(() => document.querySelector(".ptk-lock-confirm"));
        // ptk-lock-confirm is lazy loaded; process initial click.
        await tryClickLockOk(element);
        return element;
    });

    observer.observe(lockConfirm, { attributes: true, attributeFilter: ["style"] });
})();
