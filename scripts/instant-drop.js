...
// @version      0.2.22
// @description  Automatically confirm drop when droping an item.
// @author       pseudoical
...
(async function () {
    ...
    const dropBtn = await debug(`querySelector('.inv-ctxmenu-item[data-action="drop"]')`, () => {
        return getElement(() => document.querySelector('.inv-ctxmenu-item[data-action="drop"]'));
    });

    if (!(dropBtn instanceof HTMLElement)) {
        console.error("!(dropBtn instanceof HTMLElement)");
        return;
    }
