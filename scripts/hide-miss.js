...
// @version      0.2.22
// @description  Hide "MISS" text when missing attacks.
// @author       pseudoical
...
(async function () {
    ...
    const damageView = await debug(`querySelector(<damage overlay>)`, () => {
        return getElement(() => document.querySelector(
            '[style="position: fixed; inset: 0px; pointer-events: none; overflow: hidden; z-index: 100;"]'));
    });
    ...
    const observer = new MutationObserver(async (mutations) => {
        for (const mutation of mutations) {
            ...
            node.style.display = node.textContent === "MISS" ? "none" : "block";
        }
    });

    observer.observe(damageView, { childList: true, subtree: true, characterData: true });
})();
